...
CAmount getblkreward(int nPrevHeight){
    // POW
    //blockreward for testnet is 20 always
...
//}
 //   CAmount blockValue = ((nHeight % 100 == 0) && nHeight >13788) ? return blockReward * 0.999 : blockReward;
   ...
   CAmount masterNodePayment = blockValue * 0.50;
    if (nHeight > 72 && nHeight < 13140){

        masterNodePayment = blockValue * 0.50;
    }
    if (nHeight > 13141 && nHeight < 26280){
...
CAmount GetFoundersReward(int nHeight)
{
    ...
    return (blockValue >0) ? blockValue * 0.00 : 0;
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    //if (!CheckDevFundPayment(block.vtx[0], pindex->nHeight)) {
      //  mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
      //  return state.DoS(0, error("ConnectBlock(QUAR): couldn't find dev fund payment"),
